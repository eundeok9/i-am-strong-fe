...
async function findByUser(success, fail) {
  await local.get(`/user/find`).then(success).catch(fail);
}

// 사용자 정보 수정
async function userUpdate(param, success, fail) {
  await local.put(`/user/edit`, param).then(success).catch(fail);
}

// 사용자 회원탈퇴
async function userDelete(success, fail) {
  await local.delete(`/user/delete`).then(success).catch(fail);
}
export {userConfirm, naverAuth, userSingUp, findByUser, userUpdate, userDelete};
