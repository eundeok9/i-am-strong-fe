...
<template>
  <div class="container">
    <h2 class="my-3 py-3 shadow-sm bg-light text-center">
      <div class="sky">QNA</div>
    </h2>
    <div class="row justify-content-center">
      <div class="col-lg-12">
        <div class="row align-self-center mb-2">
          <div class="col-md-2 text-start" v-if="isLogin">
            <button type="button" class="btn btn-custom btn-sm" @click="moveAdd">질문하기</button>
          </div>
        </div>
        <!-- QnA Start -->
        <QnAListItem v-for="(question, index) in qna" :key="question.question_id" :question="question" :index="calculateAdjustedIndex(index)" @addBtn="callAdd" @removeBtn="callDelete"></QnAListItem>
        <!-- QnA End -->
      </div>
      <!-- 페이지네이션 Start-->
      <div class="m-4">
        <QPageNavigation v-if="totalPage > 0" :current-page="currentPage" :total-page="totalPage" @pageChange="onPageChange"></QPageNavigation>
      </div>
      <!-- 페이지네이션 End -->
    </div>
  </div>
