...
import {addAnswer, deleteQuestion} from "@/api/qna";
    ...
    <div class="accordion accordion-flush mb-3 mt-3">
      <div class="accordion-item">
        <h2 class="accordion-header">
          <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" :data-bs-target="'#accord' + question.question_id" aria-expanded="false" aria-controls="flush-collapseOne">
            <strong>#{{ index + 4 }} {{ question.title }}</strong>
          </button>
        </h2>
        <div :id="'accord' + question.question_id" class="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
          <!-- 작성 정보 시작 -->
          <div class="writerInfo">작성일: {{ question.question_create_time.split(" ")[0] }} 작성자: {{ question.writer_name }}</div>
          <!-- 작성 정보 끝 -->
          <!-- 작성 내용 시작 -->
          ...
          <div v-if="question.answer !== null">
            ...
            <!-- 관리자 답변 시작 -->
            <div class="writerInfo" v-if="question.answer_create_time !== null">답변일: {{ question.answer_create_time.split(" ")[0] }}</div>
            <div class="ans"><strong>관리자: </strong>{{ question.answer }}</div>
            <!-- 관리자 답변 끝 -->
          </div>
          <!-- 관리자 답글 작성 폼 -->
